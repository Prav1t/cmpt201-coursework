...
void groupByKey(IntermediateInput* input, Output *output, int *result_count){
    ...
    if (found_index != -1) {
        ...
        int idx = output[found_index].count;
        if (idx < MAX_INPUT) {
            output[found_index].line_numbers[idx] = input->line_number;
            output[found_index].count++;
        }
        
    } else {
        
        int new_idx = *result_count;
        output[new_idx].doubled_value = input->doubled_value;
        output[new_idx].line_numbers[0] = input->line_number;
        output[new_idx].count = 1;
        (*result_count)++;
    }
}
