...
1. What is the address of the server it is trying to connect to (IP address and port number)?
   it would be 127.0.0.1 on port 8000.

2. Is it UDP or TCP? How do you know?
   It is TCP, because the socket is created using SOCK_STREAM.

3. The client is going to send some data to the server. Where does it get this data from? 
   How can you tell in the code?
   It reads from STDIN (the keyboard) using read(STDIN_FILENO, ...).

4. How does the client program end? How can you tell that in the code?
   It ends when read() returns 0 or 1 (EOF). Then it closes the socket and exits.
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(client->cfd, buf, BUF_SIZE - 1)) > 0) {
    ...
    printf("Msg #%4d; Client ID %d: %s", msg_num, client->client_id, buf);
  }
