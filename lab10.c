...
the client is using TCP (SOCK_STREAM) over AF_INET

2. What data is the client sending to the server?

sending 5 predefined messages

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.

The run_acceptor thread receives a struct acceptor_args containing run, list_handle and list_lock.
...
main() waits until all expected messages are received then it calls collect_all() which iterates through the linked list then prints each message and then frees the node memory and message data after that counts the total collected messages

4. How are threads used in this sample code?

Main thread: Initializes data structures, creates acceptor thread, waits for messages and cleans up

Acceptor thread: Listens for incoming client connections and creates client handler threads

Client threads: One per connected client, each handles reading messages from its client socket
